...
class PdfDocument extends Document implements DocListener {
    ...
    public boolean add(Element element) throws DocumentException {
		...
		try {
			...
			case Element.PARAGRAPH: {
				...
				if (currentHeight + height[0]+height[1] + leading > indentTop() - indentBottom()) {
					newPage();
				}
			...
			case Element.TABLE: {
				...
				// <ea>
				float[] height = line.height();
				currentHeight += height[0] + height[1] - pagetop + indentTop();
				line = new PdfLine(indentLeft(), indentRight(), alignment, leading+height[1]);
				pageEmpty = false;
				break;
			}
			case Element.GIF:
    ...
    private void carriageReturn() throws DocumentException {
		...
		if (line != null) {
			...
			// <ea>
			float[] height =  line.height();
			if (currentHeight + height[0]+height[1] < indentTop() - indentBottom()) {
				// if so nonempty lines are added and the heigt is augmented
				if (line.size() > 0) {
					currentHeight += height[0]+height[1];
					lines.add(line);
					pageEmpty = false;
				}
			}
    ...
    private float flushLines() throws DocumentException {
		...
		// int currentLeading = 0; <ea>
		float prevUnderlength = 0f;
		...
		for (Iterator i = lines.iterator(); i.hasNext(); ) {
			...
			if(isParagraphE && isNewpage && newline) {
				newline=false;
				text.moveText(l.indentLeft() - indentLeft() + listIndentLeft + paraIndent,-(height[0]+prevUnderlength));
			} else {
				text.moveText(l.indentLeft() - indentLeft() + listIndentLeft, -(height[0]+prevUnderlength));
			}
			prevUnderlength = height[1];
			...
			// <ea>
			displacement += height[0] + height[1];
			...
			if (chunkStrokeIdx <= lastChunkStroke) {
				...
				if (chunk.isStroked()) {
					...
					if (chunk.isAttribute(Chunk.ACTION)) {
						...
						if (nextChunk == null)
							subtract += hangingCorrection;
						annotations.add(new PdfAnnotation(writer, xMarker, yMarker,
					...
					if (chunk.isAttribute(Chunk.REMOTEGOTO)) {
						...
						if (obj[1] instanceof String)
							remoteGoto(filename, (String)obj[1], xMarker, yMarker,
									   xMarker + width - subtract, yMarker + chunk.font().size());
						else
							remoteGoto(filename, ((Integer)obj[1]).intValue(), xMarker, yMarker,
									   xMarker + width - subtract, yMarker + chunk.font().size());
					}
