...
/**
 * 
 * @author <a href="mailto:prakashohm@gmail.com">Ohmprakash Balaiah</a>
 */
public class TamilGlyphRepositioner extends IndicGlyphRepositioner {
	
	private static final String[] CHARCTERS_TO_BE_SHIFTED_LEFT_BY_1 = new String[] {
		"\u0B82", "\u0BC1","\u0BC2","\u0BC6","\u0BC7","\u0BC8","\u0BCA","\u0BCB","\u0BCC","\u0BCD","\u0BD7"};
	
	private final Map<Integer, int[]> cmap31;
	private final Map<String, Glyph> glyphSubstitutionMap;

	public TamilGlyphRepositioner(Map<Integer, int[]> cmap31, Map<String, Glyph> glyphSubstitutionMap) {
		this.cmap31 = cmap31;
	...
	public void repositionGlyphs(List<Glyph> glyphList) {
		...
		for (int i = 0; i < glyphList.size(); i++) {
			...
			if (glyph.chars.equals("\u0BCA")) {
				handleOKaarAndOUKaar(i, glyphList, '\u0BC6', '\u0BBE');
			} else if (glyph.chars.equals("\u0BCB")) {
				handleOKaarAndOUKaar(i, glyphList, '\u0BC7', '\u0BBE');
			} else if (glyph.chars.equals("\u0BCC")) {
				handleOKaarAndOUKaar(i, glyphList, '\u0BC6', '\u0BD7');
			}
		}
