	...
	for(int j = 0; j < currentRow.columns(); j++)
	  {
	    if(emptyCells.indexOf(new String(j+"-"+i+"h")) == -1)
	      {
		if((currentRow.getCell(j) != null) && (currentRow.getCell(j).colspan() != 1))
		  {
		    rows[i].write(escape);
		    rows[i].write(mergeHorizontalBegin);
		    for(int k = j + 1; k <= j + currentRow.getCell(j).colspan()-1; k++)
	    ...
	    if(emptyCells.indexOf(new String(j+"-"+i+"v")) == -1)
	      {
		if((currentRow.getCell(j) != null) && (currentRow.getCell(j).rowspan() != 1))
		  {
		    rows[i].write(escape);
		    rows[i].write(mergeVerticalBegin);
		    for(int k = i + 1; k <= i + currentRow.getCell(j).rowspan()-1; k++)
	    ...
	    if(currentRow.getCell(j) != null)
	      {
		rows[i].write(escape);
		switch(currentRow.getCell(j).verticalAlignment())
		  {
	...
	for(int j = 0; j < currentRow.columns(); j++)
	  {
	    if((emptyCells.indexOf(new String(j+"-"+i+"h")) == -1) && (emptyCells.indexOf(new String(j+"-"+i+"v")) == -1))
	      {
		for(cellIterator = currentRow.getCell(j).getElements(); cellIterator.hasNext();)
		  {
		    ...
		    rows[i].write(inTableBegin);
		    rows[i].write(escape);
		    switch(currentRow.getCell(j).horizontalAlignment())
		      {
