 ...
 */
    ...
    Type1Font(String afmFile, String enc, boolean emb, byte ttfAfm[], byte pfb[], boolean forceRead)
        ...
        if (emb && ttfAfm != null && pfb == null)
            throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "two.byte.arrays.are.needed.if.the.type1.font.is.embedded"));
        if (emb && ttfAfm != null)
            this.pfb = pfb;
        encoding = enc;
        embedded = emb;
        ...
        if (BuiltinFonts14.containsKey(afmFile)) {
            ...
            try {
                ...
                if (is == null) {
                    String msg = MessageLocalization.getComposedMessage(this.getClass(), "1.not.found.as.resource", afmFile);
            ...
            throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "1.is.not.an.afm.or.pfm.font.file", afmFile));
 ...
 */
    ...
    public void process(RandomAccessFileOrArray rf) throws DocumentException, IOException
    {
        ...
        if (!isMetrics)
            throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "missing.startcharmetrics.in.1", fileName));
        while ((line = rf.readLine()) != null)
        {
            StringTokenizer tok = new StringTokenizer(line);
            if (!tok.hasMoreTokens())
        ...
        if (isMetrics)
            throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "missing.endcharmetrics.in.1", fileName));
        ...
        if (!isMetrics)
            throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "missing.endfontmetrics.in.1", fileName));
        while ((line = rf.readLine()) != null)
        {
            StringTokenizer tok = new StringTokenizer(line);
            if (!tok.hasMoreTokens())
        ...
        if (isMetrics)
            throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "missing.endkernpairs.in.1", fileName));
        rf.close();
    }
 ...
 */
    public PdfStream getFullFontStream() throws DocumentException
    {
        ...
        try {
            ...
            for (int k = 0; k < 3; ++k) {
                if (rf.read() != 0x80)
                    throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "start.marker.missing.in.1", filePfb));
                if (rf.read() != PFB_TYPES[k])
                    throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "incorrect.segment.type.in.1", filePfb));
                int size = rf.read();
                size += rf.read() << 8;
                size += rf.read() << 16;
                size += rf.read() << 24;
                ...
                while (size != 0) {
                    ...
                    if (got < 0)
                        throw new DocumentException(MessageLocalization.getComposedMessage(this.getClass(), "premature.end.in.1", filePfb));
                    bytePtr += got;
                    size -= got;
                }
            }
