...
public class PdfShading {
    ...
    public static void throwColorSpaceError() {
        throw new IllegalArgumentException(MessageLocalization.getComposedMessage(PdfShading.class, "a.tiling.or.shading.pattern.cannot.be.used.as.a.color.space.in.a.shading.pattern"));
    }
    ...
    public static void checkCompatibleColors(Color c1, Color c2) {
        ...
        if (type1 != type2)
            throw new IllegalArgumentException(MessageLocalization.getComposedMessage(PdfShading.class, "both.colors.must.be.of.the.same.type"));
        if (type1 == ExtendedColor.TYPE_SEPARATION && ((SpotColor)c1).getPdfSpotColor() != ((SpotColor)c2).getPdfSpotColor())
            throw new IllegalArgumentException(MessageLocalization.getComposedMessage(PdfShading.class, "the.spot.color.must.be.the.same.only.the.tint.can.vary"));
    ...
    public void setBBox(float[] bBox) {
        if (bBox.length != 4)
            throw new IllegalArgumentException(MessageLocalization.getComposedMessage(this.getClass(), "bbox.must.be.a.4.element.array"));
        this.bBox = bBox;
    }
